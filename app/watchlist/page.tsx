...
export default async function WatchlistPage({ searchParams }: PageProps) {
  ...
  // Watchlist itu data privat milik user, jadi wajib login
  if (!user) {
    redirect('/login?redirectTo=/watchlist');
  }

  // 1. Ambil watchlist milik user ini.
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12 space-y-4">
          <div className="bg-white rounded-3xl border border-border-soft p-6 sm:p-8 text-center space-y-4 shadow-sm">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-text-muted">Belum ada komoditas yang kamu pantau saat ini.</p>
            </div>
            <div>
              <Link
                href="/dashboard"
  ...
  // 2. Ambil semua harga untuk komoditas yang di-watch
  ...
  // Agregat rata-rata lintas pasar per komoditas
  const aggregatedByCommodity = new Map(
    aggregateByCommodity(rows).map((a) => [a.commodityId, a])
  );

  // Laporan terbaru per kombinasi commodity+market
  ...
  // Rata-rata 7 hari terakhir per NAMA komoditas
  ...
  // 4. Hitung pagination metadata & slice items
  ...
  return (
    <div className="bg-bg-organic text-text-main min-h-screen font-sans antialiased">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-6">
        ...
        {/* Content Box */}
        <div className="bg-white rounded-3xl border border-border-soft p-4 sm:p-6 shadow-sm space-y-6">
          {/* DESKTOP TABLE */}
            ...
            <table className="w-full text-sm text-left border-collapse">
              ...
              <tbody className="divide-y divide-border-soft/60">
                {paginatedViews.map((item) => (
                  <tr key={`${item.commodityId}:${item.marketId ?? 'null'}`} className="hover:bg-bg-organic transition-colors">
                    ...
                    <td className="py-3.5 pr-4 text-xs text-text-muted">{item.sourceLabel}</td>
          ...
          <div className="md:hidden space-y-3">
            {paginatedViews.map((item) => (
              <div
                ...
                className="p-4 rounded-2xl bg-bg-organic border border-border-soft space-y-3"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-bold text-base text-text-main">{item.commodityName}</h3>
                    <p className="text-xs text-text-muted">📍 {item.sourceLabel}</p>
                  </div>
                  <WatchlistButton
                    commodityId={item.commodityId}
                    marketId={item.marketId}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border-soft/60 text-xs">
                  <div>
                    <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold">Harga Terbaru</p>
